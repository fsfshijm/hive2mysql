...
FIND_PARTITION_SQL = "SELECT * FROM information_schema.PARTITIONS WHERE table_name='impression_summary' and partition_name=%s;"

ADD_PARTITION_SQL = "ALTER TABLE impression_summary ADD PARTITION(PARTITION %s VALUES in (%d));"
...
SELECT
    ...
    count(distinct (searchid)) as request, 
    count(distinct (case when not isspam then searchid else cast (0 as bigint) end ))-1 as erequest, 
    count(distinct (case when adcreativeid > 0 then concat(searchid, mediaid, adcreativeid, impressiontime) else NULL end)) as impression, 
    count(distinct (case when not isspam and  adcreativeid > 0 then concat(searchid, mediaid, adcreativeid, impressiontime) else NULL end)) as eimpression, 
    dt,
    hr
...
INSERT_DATA_SQL = """
INSERT INTO impression_summary
SET
    mediaid = '%s',
    costtype = '%s',
    sdk_platform = '%s',
...
def load_into_mysql(time):
    ...
    query_result = query_by_hive_cmd(GROUP_HQL % ('imp_total',dt, hr), 'bi')


    bi_db = Connection(host='dreportm.db.in.xxxxxxxxx.cn', database='bi', user='bi', password='bibi')
    bi_db.execute("DELETE FROM impression_summary WHERE (dt='%s' AND hr=%d) or (dt='%s') " % (formated_date, int(hr), days_ago_30))
    ...
    print "%s rows inserted into impression_summary" % counter
