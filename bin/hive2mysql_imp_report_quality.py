...
FIND_PARTITION_SQL = "SELECT * FROM information_schema.PARTITIONS WHERE table_name='imp_report_quality_summary' and partition_name=%s;"

ADD_PARTITION_SQL = "ALTER TABLE imp_report_quality_summary ADD PARTITION(PARTITION %s VALUES in (%d));"
...
SELECT
    ...
    count(distinct (case when phase='s' then concat(searchid, mediaid, adcreativeid, impressiontime) else NULL end)) as imp_report_count,
    count(distinct (case when not isspam and  phase='s' then concat(searchid, mediaid, adcreativeid, impressiontime) else NULL end)) as eimp_report_count, 
    dt,
    hr
...
INSERT_DATA_SQL = """
INSERT INTO imp_report_quality_summary
SET
    mediaid = '%s',
    costtype = '%s',
    sdk_platform = '%s',
...
def load_into_mysql(time):
    ...
    bi_db.execute("DELETE FROM imp_report_quality_summary WHERE (dt='%s' AND hr=%d) or (dt='%s') " % (formated_date, int(hr), days_ago_30))
    ...
    print "%s rows inserted into imp_report_quality_summary" % counter
